package com.sekai.contentprovider.database
...
import com.sekai.contentprovider.database.NotesDatabaseHelper.Companion.TABLE_NOTES
import java.net.URI
...
class NoteProvider : ContentProvider() {
    ...
    override fun getType(uri: Uri): String? = throw UnsupportedSchemeException("nao implementado")

    override fun insert(uri: Uri, values: ContentValues?): Uri? {
        if (mUriMatcher.match(uri) == NOTES){
            val db : SQLiteDatabase = dbHelper.writableDatabase
            val id = db.insert(TABLE_NOTES, null, values)
            val insetUri = Uri.withAppendedPath(BASE_URI, id.toString())
            db.close()
            context?.contentResolver?.notifyChange(uri, null)
            return insetUri
    ...
    companion object {
        const val AUTHORITY = "com.sekai.contentprovider.provider"
